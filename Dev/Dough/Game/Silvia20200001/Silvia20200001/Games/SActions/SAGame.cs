...
namespace Charlotte.Games.SActions
{
	...
	/// ゲームメイン
	/// </summary>
	public class SAGame : IDisposable
	{
		public static SAGame I;
		...
		public void Run(SAField field)
		{
			...
			Inputs.D.FreezeInputUntilRelease();
			...
			for (this.Frame = 0; ; this.Frame++)
			{
				...
				if (ProcMain.DEBUG && Inputs.START.GetInput() == 1)
				{
					this.DebugPause();
				}
				...
				// プレイヤー入力
				{
					...
					else // ? 接地中 || 滞空中
					{
						...
						else // ? 接地状態からのジャンプが「可能ではない」状態
						{
							...
							if (1 <= jump && jump < 事前入力時間 && this.Player.JumpCount < SAConsts.PLAYER_JUMP_MAX && !this.Player.JumpLock)
							{
								...
								DD.EL.Add(SCommon.Supplier(SAEffects.空中ジャンプの足場(this.Player.X, this.Player.Y + SAConsts.PLAYER_接地判定Pt_Y)));
					...
					{
						SAGame.I.Shots.Add(new SAShot_Test0001(this.Player.X, this.Player.Y, this.Player.FacingLeft));
					}
					//*/
				}
				...
				if (1 <= this.Player.DamageFrame) // ? プレイヤー・ダメージ中
				{
					...
					// プレイヤー・ダメージ中の処理
					{
						if (frame == 2) // 初回のみ
						{
							DD.EL.Add(SCommon.Supplier(SAEffects.Explode(this.Player.X, this.Player.Y, 2.0)));
		...
		endOfGameLoop:
			...
			if (this.ExitDirection == 901) // ? 死亡によりゲーム終了
			{
				...
				DD.EL.Add(SCommon.Supplier(SAEffects.Explode(this.Player.X, this.Player.Y, 5.0)));
