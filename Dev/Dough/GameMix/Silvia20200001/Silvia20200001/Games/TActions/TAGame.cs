...
namespace Charlotte.Games.TActions
{
	...
	/// ゲームメイン
	/// </summary>
	public class TAGame : IDisposable
	{
		public static TAGame I;
		...
		public void Run(TAField field)
		{
			...
			for (this.Frame = 0; ; this.Frame++)
			{
				...
				// プレイヤー入力・移動
				{
					...
					bool fast =
						!damageInputLock && 1 <= Inputs.R.GetInput();
					bool slow =
					...
					if (fast)
						speed *= TAConsts.PLAYER_FAST_SPEED_RATE;

					if (slow)
						speed *= TAConsts.PLAYER_SLOW_SPEED_RATE;
		...
		endOfGameLoop:
			...
			if (this.ExitDirection == 901) // ? 死亡によりゲーム終了
			{
				...
				DD.EL.Add(SCommon.Supplier(TAEffects.Explode(this.Player.X, this.Player.Y, 5.0)));
